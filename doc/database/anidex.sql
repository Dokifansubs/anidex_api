...
CREATE TABLE IF NOT EXISTS `anidex`.`torrent_stats` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `info_hash_UNIQUE` (`info_hash` ASC),
  CONSTRAINT `fk_torrent_torrent_stats`
    FOREIGN KEY (`info_hash`)
    REFERENCES `anidex`.`torrent` (`info_hash`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
ON `torrent`.`info_hash` = `torrent_stats`.`info_hash`
INNER JOIN `category`
...
USE `anidex`$$
DROP TRIGGER IF EXISTS `anidex`.`torrent_AINS` $$
USE `anidex`$$
CREATE TRIGGER `torrent_AINS` AFTER INSERT ON `torrent` FOR EACH ROW
BEGIN
	INSERT INTO `torrent_stats` (`info_hash`) VALUES(NEW.`info_hash`);
END$$
